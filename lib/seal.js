 ...
 * AES-128 in CBC mode with ingretiy provided by a SHA-256 HMAC.
 *
  ...
  return function fernet_seal(claims, options, cb) {
    ...
    keying(query, function(err, encryptionKeys) {
      ...
      if (!key.usages || key.usages.indexOf('sign') !== -1) {
        // Either there is no restirctions on key usage, or the key is
        // explicitly allows signing (and encryption as implied by the query).
        // Proceed to use the same key for both encryption and signing.
      ...
      keying(query, function(err, signingKeys) {
        if (err) { return cb(err); }
        return proceed(key, signingKeys[0]);
      });
    });
  };
};
