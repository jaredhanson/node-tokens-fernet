...
module.exports = function(options, keying) {
  ...
  return function fernet_unseal(sealed, parameters, cb) {
    ...
    function proceed(encryptionKeys, signingKeys) {
      ...
      var key = signingKeys.secret + encryptionKeys.secret;
    ...
    keying(parameters.issuer, query, function(err, keys) {
      ...
      keying(parameters.issuer, query, function(err, signingKeys) {
        if (err) { return cb(err); }
